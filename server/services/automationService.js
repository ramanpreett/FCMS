...
class AutomationService {
  ...
  async parseEmailContent(emailContent) {
    ...
    try {
      console.log('Starting email content parsing...');
      ...
      const extractedPhone = this.extractPhoneFromContent(emailContent);
      ...
      console.log('- Tags:', extractedTags);
  ...
  // Accept optional userId so generated clients can be assigned to a user
  async generateClientProfile(clientData, userId = null) {
    try {
      const clientPayload = {
      ...
      // Assign owner when provided
      if (userId) {
        clientPayload.user = userId;
      }
  ...
  extractPhoneFromContent(content) {
    try {
      ...
      console.log('Content to search:', content);
      
      // Simple and comprehensive regex patterns
      const allPhonePatterns = [
      ...
      for (const pattern of allPhonePatterns) {
        const matches = [...content.matchAll(pattern)];
        matches.forEach(match => {
          const number = match[1].trim();
          console.log('Potential phone found:', number);
          foundNumbers.push(number);
        });
      }
      
      console.log('All potential numbers found:', foundNumbers);
      
      // Filter and validate
      const validNumbers = foundNumbers.filter(num => {
        const digitsOnly = num.replace(/\D/g, '');
        const digitCount = digitsOnly.length;
        console.log(`Validating ${num} -> ${digitsOnly} (${digitCount} digits)`);
        
        // Must have 7-15 digits
        if (digitCount >= 7 && digitCount <= 15) {
          // Extra validation for common patterns
          if (digitCount === 10) {
            // Indian mobile (starts with 6-9) or US number
            return /^[6-9]/.test(digitsOnly) || /^[2-9]/.test(digitsOnly);
          }
          return true;
        }
        return false;
      ...
      if (validNumbers.length > 0) {
        ...
        const digitsOnly = selectedNumber.replace(/\D/g, '');
        
        console.log('Selected number:', selectedNumber);
        console.log('Digits only:', digitsOnly);
        
        // Format based on digit count and pattern
        if (digitsOnly.length === 10) {
          if (/^[6-9]/.test(digitsOnly)) {
            const formatted = `+91 ${digitsOnly.slice(0,5)} ${digitsOnly.slice(5)}`;
            console.log('Formatted as Indian mobile:', formatted);
            return formatted;
          } else if (/^[2-9]/.test(digitsOnly)) {
        ...
        // For other cases, return the original with basic formatting
        const basicFormatted = selectedNumber.replace(/\s+/g, ' ').trim();
        console.log('Basic formatted:', basicFormatted);
